...
struct Benchmark {
  ...
  void run() {
    ...
    CATCH_TRY {
      ...
      BenchmarkStats<std::chrono::duration<double, std::nano>> stats{info,
                                                                     analysis.samples,
                                                                     analysis.mean,
...
std::unique_ptr<EnumInfo> makeEnumInfo(StringRef enumName, StringRef allValueNames,
  ...
  std::size_t i = 0;
  for (auto value : values) enumInfo->m_values.push_back({value, valueNames[i++]});
...
void SectionTracker::addNextFilters(std::vector<std::string> const& filters) {
  if (filters.size() > 1) m_filters.insert(m_filters.end(), ++filters.begin(), filters.end());
}
...
void TestSpecParser::visitChar(char c) {
  ...
  switch (m_mode) {
    ...
    case Tag:
    case QuotedName:
      if (processOtherChar(c)) return;
      break;
  }

  m_substring += c;
  if (!isControlChar(c)) m_patternName += c;
}
